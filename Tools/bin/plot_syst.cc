...
TH1F* plot_one(int year, int numSigma, bool endcap, const std::string& syst_type, const std::string& plotVar, bool unweighted = false);
...
void plot_syst(const std::string & syst_type = "egm")
{
  ...
  for (std::string plotVar : {"mgg", "pt", "nPV"}) {
    for (int year : {2016, 2017, 2018}) {
      for (bool endcap : {true, false} ) {
        ...
        TH1F * up1sigma = plot_one(year, 1, endcap, syst_type, plotVar);
        TH1F * nominal = plot_one(year, 0, endcap, syst_type, plotVar);
        TH1F * down1sigma = plot_one(year, -1, endcap, syst_type, plotVar);
        TH1F * unweighted = plot_one(year, 0, endcap, syst_type, plotVar, true);
        ...
        if (plotVar == "mgg") {
          ...
          nominal->SetTitle(";P_{t}^{#gamma_{1}} (GeV);Photons/(50 GeV)");
        }
        else if (plotVar == "nPV") {
          nominal->SetTitle(";nPV;Photons/(20)");
        }
        ...
        for(int i=0; i<=ratio->GetNbinsX(); i++) {
          ...
          if(content != 0) {
            ...
            std::cout << i << " " << content << " " << error << " " << error/content << " beforeweighting: " << unweighted->GetBinContent(i) << " " << unweighted->GetBinError(i) << " " << sqrt(unweighted->GetBinContent(i)) << std::endl;
          }
          else {
        ...
        std::string filename(plotVar + "_" + syst_type + "_syst_gg_");
...
TH1F* plot_one(int year, int numSigma, bool endcap, const std::string& syst_type, const std::string& plotVar, bool unweighted)
{
  ...
  std::string id_cuts("Photon1.passHighPtID && Photon2.passHighPtID");
  ...
  TH1F* hist;
  if (plotVar != "nPV") {
    hist = new TH1F(histName.c_str(), histName.c_str(), 40, 0, 2000);
  }
  else {
    hist = new TH1F(histName.c_str(), histName.c_str(), 50, 0, 100);
  }
  ...
  if (plotVar == "mgg") {
    ...
    chains["gg_" + std::to_string(year)]->Draw(("Photon1.pt>>" + histName).c_str(), allCuts.Data(), "hist");
  }
  else if (plotVar == "nPV") {
    chains["gg_" + std::to_string(year)]->Draw(("nPV>>" + histName).c_str(), allCuts.Data(), "hist");
  }
